...
function App() {
  ...
  useEffect(() => {
    window.addEventListener("scroll", () => {
      ...
      bannerRef.current.style.transform = `scale(${1 + value * 0.0005})`
      bg_11Ref.current.style.transform = `scale(${1 + value * 0.0003}px)`

      h3Ref.current.style.transform = `translateZ(${value}px) scale(${1 + (value * 0.0006)})`
      h1Ref.current.style.transform = `translateY(${value * 0.8}px)`
      console.log();
